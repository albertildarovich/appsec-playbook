...
```java
// [NO] Проверена только аутентификация
...
- [OK] Пользователь вошёл в систему

Но не проверило:

- [NO] Имеет ли он право читать **именно этот объект**
...
```java
// [NO] ОПАСНО — проверяем только существование
Order order = orderRepo.findById(id);
if (order != null) {
    return order;
}

// [OK] БЕЗОПАСНО — проверяем право доступа
Order order = orderRepo.findByIdAndOwnerId(id, currentUserId);
if (order != null) {
    return order;
}

// [OK] ЕЩЁ ЛУЧШЕ — через сервис авторизации
...
```sql
-- [NO] ОПАСНО — любой может запросить любой id
SELECT * FROM orders WHERE id = :id;

-- [OK] БЕЗОПАСНО — фильтр по владельцу
...
# Сервер проверяет токен ([OK]), но не проверяет роль ([NO])
```
...
```sql
-- [NO] ОПАСНО — данные не изолированы по tenant
...
| Детализация | [OK] Роль | [OK] Атрибуты + контекст |
| Внедрение | [OK] Простое | [NO] Сложное |
| Поддержка | [NO] Сложно при росте | [OK] Легче при правильной архитектуре |
...
```java
// [NO] Проверки в каждом контроллере — дублирование и риск
@GetMapping("/orders/{id}")
...
- [OK] Единая логика
- [OK] Меньше ошибок (не забыли проверку)
- [OK] Проще сопровождать
- [OK] Defense in Depth
- [OK] Аудит (можно логировать все решения)
...
```java
// [NO] ОПАСНО — пользователь управляет всеми полями
@PostMapping("/users")
...
```java
// [OK] БЕЗОПАСНО — DTO содержит только разрешённые поля
...
```java
// [NO] Бесполезный DTO — повторяет Entity
public record CreateUserDTO(
    String name,
    String email,
    String password,
    String role,     // [NO] не должно быть от клиента
    BigDecimal balance,  // [NO] не должно быть от клиента
    String status   // [NO] не должно быть от клиента
) {}
```
...
## Что дальше
- [x] **Cryptographic Failures (A02)** — [OK] пройдено
